    ...
    for ckpt in ckpts_to_eval:
        ...
        for train_run in range(vcfg['n_repeats']):
            ...
            for i in range(0, n_demos):
                ...
                for i in range(20):
                    import ipdb;ipdb.set_trace()
                     # move the yellow ring to the darker brown side
                    info['lang_goal'] = lang_goal
                    env.task.lang_goals[0] = lang_goal
                    if record:
                        updated_video_name = f"{task_name}---{lang_goal}-step-{i}"# Update the video name
                    ...
                    # print(f'Total Reward: {total_reward:.3f} | Done: {done}\n')
                        # Update and restart video recording
                    if record:

...
def get_pointcloud(depth, intrinsics):
    ...
    px = (px - intrinsics[0, 2]) * (depth / intrinsics[0, 0])  # (u - px) * z / fx
    py = (py - intrinsics[1, 2]) * (depth / intrinsics[1, 1])  # (v - py) * z / fy
...
def reconstruct_heightmaps_real(color, depth, configs, bounds, pixel_size, pointcloud):
    """Reconstruct top-down heightmap views from multiple 3D pointclouds."""
    heightmaps, colormaps = [], []
    for color, depth, pointcloud, config in zip(color, depth, pointcloud, configs):
        intrinsics = np.array(config['intrinsics']).reshape(3, 3)
        xyz = pointcloud # get_pointcloud(depth, intrinsics) # pointcloud
        ...
        # import ipdb;ipdb.set_trace() # 아예 부호가 반대인 이유 찾기
...
def get_fused_heightmap_real(obs, configs, bounds, pix_size):
    """Reconstruct orthographic heightmaps with segmentation masks."""
    heightmaps, colormaps = reconstruct_heightmaps_real(
        obs['color'], obs['depth'], configs, bounds, pix_size, obs['pointcloud'])

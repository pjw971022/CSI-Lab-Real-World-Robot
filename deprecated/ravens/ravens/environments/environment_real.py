...
class RealEnvironment(gym.Env):
    ...
    def __init__(self,
        ...
        self.audio_file_path = "/Sembot/real_bot/perception/speech_command.wav"
    ...
    def get_data(self, config, send_msg, image_size=None, reset=False):
        if  isinstance(send_msg, str) and "human-cam" in send_msg:    
            ...
            if send_msg == "require human-cam image":
                ...
                with open(WORKSPACE + '/Sembot/real_bot/save_vision/obs/human_image.png', 'wb') as image_file:
                    image_file.write(image_data)
            elif send_msg == "require human-cam video":
                video_data = self.socket_robot.recv()
                print("human-cam client received!!")

                with open(WORKSPACE + '/Sembot/real_bot/save_vision/obs/human_video.mp4', 'wb') as video_file:
            ...
            cv2.imwrite(fWORKSPACE + '/Sembot/real_bot/save_vision/obs/image_obs_{self.step_counter}.png', color)
            if not reset:
                with open(f"/Sembot/real_bot/save_vision/obs/gripper_video_{self.step_counter}.mp4", "wb") as video_file:

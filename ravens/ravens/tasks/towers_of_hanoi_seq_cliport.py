...
class TowersOfHanoiSeqCliport(Task):
    ...
    def reset(self, env):
        ...
        for i in range(n_disks):
            ...
            disks_names[disk_id] = color_names[i] + ' ring'
            self.ind2obj[disk_id] = color_names[i] + ' ring'

        # sample disk (hardest one is lowest disk)
        all_rods_ids = [21, 22]
        if self.task_level == 3:
            pick_disk_id = list(disks_names.keys())[-1]
            place_rod_id = 22
        else:
            pick_disk_id = list(disks_names.keys())[-1] # np.random.choice(list(disks_names.keys()))
        ...
        self.observation()
        self.lang_initial_state = f'{self.lang_observation}The rings can be moved in ' \
                                  f'lighter brown side, middle of the stand, darker brown side. '
        
        self.lang_initial_state = self.lang_initial_state.replace('disk','ring')

        self.adm_act = \
            [f"move {ring} in {rod}" for ring, rod in product(disks_names.values(), rod_names)]
        self.admissible_actions = [act.replace('disk','ring')  for act in self.adm_act]
